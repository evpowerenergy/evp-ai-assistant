-- AI Assistant RPC Functions
-- Migration: 20250116000003_ai_rpc_functions.sql
-- These functions connect to CRM database tables

-- Function: Get Lead Status
-- Searches for lead by name and returns status information
...
BEGIN
    ...
    -- Search for lead by name (full_name or display_name)
    SELECT 
        id,
        full_name,
        display_name,
    ...
    FROM leads
    WHERE (
        LOWER(full_name) LIKE LOWER('%' || p_lead_name || '%')
        OR LOWER(display_name) LIKE LOWER('%' || p_lead_name || '%')
    )
    AND is_archived = false
    ORDER BY updated_at_thai DESC
    LIMIT 1;
    ...
    IF v_user_role IN ('admin', 'manager') THEN
        ...
        v_result := jsonb_build_object(
            ...
            'status', v_lead_record.status,
            'category', v_lead_record.category,
            'region', v_lead_record.region,
            'sale_owner_id', v_lead_record.sale_owner_id,
            'created_at', v_lead_record.created_at_thai,
            'updated_at', v_lead_record.updated_at_thai,
            'operation_status', v_lead_record.operation_status,
            'notes', v_lead_record.notes
        );
    ELSIF v_user_role = 'staff' THEN
        -- Check if lead belongs to user
        -- Note: This requires checking sale_owner_id against user's sales_team record
...
GRANT EXECUTE ON FUNCTION ai_get_team_kpi(UUID, INTEGER) TO authenticated;
