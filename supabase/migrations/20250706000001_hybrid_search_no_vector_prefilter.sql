-- Hybrid search: do not pre-filter vector hits by similarity threshold.
-- Threshold is kept for API compatibility but retrieval uses top-N vector + keyword RRF.

CREATE OR REPLACE FUNCTION public.hybrid_search_kb(
    query_embedding vector(1536),
    query_text text,
    match_count int DEFAULT 20,
    similarity_threshold float DEFAULT 0.38,
...
BEGIN
    ...
    FROM combined
    WHERE cfused > 0
      AND (cvscore >= similarity_threshold OR ckscore > 0 OR cfused >= 0.01)
