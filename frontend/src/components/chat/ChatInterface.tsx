...
export function ChatInterface() {
  ...
  const { currentSession, sessions, createSession, switchSession, deleteSession } = useSession()
  ...
  return (
    <div className="flex h-full min-h-0 max-w-full overflow-hidden bg-gray-50">
      {/* Session Sidebar */}
      <SessionSidebar
        open={sidebarOpen}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Header */}
        <header className="flex flex-shrink-0 items-center justify-between gap-2 border-b bg-white px-4 py-2">
          ...
          <button
            ...
            className="hidden shrink-0 rounded-md p-2 text-gray-600 hover:bg-gray-100 lg:block"
          >
            <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
            </svg>
          ...
          {isLoadingHistory ? (
            <div className="flex h-full items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600">กำลังโหลดประวัติการสนทนา...</p>
              </div>
            </div>
          ) : (
            <>
